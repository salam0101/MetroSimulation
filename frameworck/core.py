...
class Controller(frameworck.interfaces.IController):
    """
    ...
    def registerCommand(self, notificationName, commandClassRef):
        ...
        if (self.commandMap.get(notificationName,None) == None):
            self.view.registerObserver(notificationName, frameworck.patterns.observer.Observer(self.executeCommand, self))
...
class Model(frameworck.interfaces.IModel):
    """
...
class View(frameworck.interfaces.IView):
    """
    ...
    def registerMediator(self, mediator):
        ...
        if len(interests) > 0:
            obsvr = frameworck.patterns.observer.Observer(mediator.handleNotification, mediator)
