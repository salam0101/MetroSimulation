"""
 frameworck Python Port by Toby de Havilland <toby.de.havilland@frameworck.org> 
 frameworck - Copyright(c) 2006-08 Futurescale, Inc., Some rights reserved. 
 Your reuse is governed by the Creative Commons Attribution 3.0 License 
"""

import frameworck.interfaces
import frameworck.patterns.facade

class Observer(frameworck.interfaces.IObserver):
    """
    ...
    In frameworck, the C{Observer} class assumes these responsibilities:
    ...
    @see: L{View<org.frameworck.as3.core.view.View>}
    @see: L{Notification<org.frameworck.as3.patterns.observer.Notification>}
    """
    notify = None
    context = None
...
class Notifier(frameworck.interfaces.INotifier):
    """
    ...
    @see: L{Facade<org.frameworck.as3.patterns.facade.Facade>}
    @see: L{Mediator<org.frameworck.as3.patterns.mediator.Mediator>}
    @see: L{Proxy<org.frameworck.as3.patterns.proxy.Proxy>}
    @see: L{SimpleCommand<org.frameworck.as3.patterns.command.SimpleCommand>}
    @see: L{MacroCommand<org.frameworck.as3.patterns.command.MacroCommand>}
    """
    ...
    def __init__(self):
        ...
        """
        self.facade = frameworck.patterns.facade.Facade.getInstance()
...
class Notification(frameworck.interfaces.INotification):
    """
    A base C{INotification} implementation.
    
    frameworck does not rely upon underlying event models such 
    as the one provided with Flash, and ActionScript 3 does 
    not have an inherent event model.</P>
    
    The Observer Pattern as implemented within frameworck exists 
    ...
    A key difference between Flash C{Event}s and frameworck 
    C{Notification}s is that C{Event}s follow the 
    'Chain of Responsibility' pattern, 'bubbling' up the display hierarchy 
    until some parent component handles the C{Event}, while
    frameworck C{Notification}s follow a 'Publish/Subscribe'
    pattern. frameworck classes need not be related to each other in a 
    parent/child relationship in order to communicate with one another
    using C{Notification}s.
    
    @see: L{Observer<org.frameworck.as3.patterns.observer.Observer>}
    """
